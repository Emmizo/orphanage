...
// if (!empty($_POST['submit'])) {
    ...
    // $user_id=$_GET['sponsor_id'];
    $status=$_GET['status'];
$query=mysqli_query($conn,"UPDATE orphan SET `status`='4' WHERE `id`='$id'" )or die(mysqli_error($conn));
$query="INSERT INTO sponsor_orphan (`orphan_id`,`sponsor_id`,`status`) VALUES('$id','$user_id','4')" ;
 $result=mysqli_query($conn,$query)or die(mysqli_error($conn));
header("location:index.php");
// }
      ?>
